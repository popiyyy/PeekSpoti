...
@section('content')
    ...
    @else
        <div class="space-y-12">
            <!-- Profil Pengguna -->
            <div
                class="flex flex-col md:flex-row items-center gap-6 bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800/80 shadow-2xl backdrop-blur-sm">
                @if($user->avatar_url)
                    <img src="{{ $user->avatar_url }}" alt="Avatar"
                        class="w-28 h-28 rounded-full shadow-lg ring-4 ring-zinc-800/50">
                @else
                ...
                <div class="md:ml-auto">
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="px-5 py-2.5 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 hover:text-red-300 text-sm font-semibold rounded-full transition-all flex items-center gap-2">
            ...
            <div class="space-y-6">
                ...
                <!-- Tampilan AI Gemini -->
                @if(isset($aiAnalysis))
                <div class="bg-linear-to-r from-emerald-900/40 to-blue-900/40 p-6 rounded-3xl border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.1)] relative overflow-hidden group mb-8">
                    <div class="absolute -right-10 -top-10 text-emerald-500/10 transition-transform duration-700 group-hover:rotate-12 group-hover:scale-110">
                        <svg class="w-48 h-48" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                        </svg>
                    </div>
                    <h3 class="text-xl font-bold text-emerald-400 flex items-center gap-2 mb-3">
                ...
                @if($artists && count($artists) > 0)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($artists as $name => $count)
                            <div
                                class="bg-zinc-900/60 p-4 rounded-2xl border border-zinc-800 hover:border-emerald-500/50 transition-colors group">
                                ...
                                <div class="w-full bg-zinc-950 rounded-full h-2.5 overflow-hidden">
                                    <div class="bg-emerald-500 h-2.5 rounded-full transition-all duration-1000 ease-out"
